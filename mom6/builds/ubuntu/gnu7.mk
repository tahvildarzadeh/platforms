...
FFLAGS += $(shell pkg-config --cflags-only-I mpi)
FFLAGS_OPT = -O3 
...
CFLAGS += $(shell pkg-config --cflags-only-I mpi)
CFLAGS_OPT = -O2
...
LIBS := $(shell pkg-config --libs-only-L netcdf) $(shell pkg-config --libs-only-L mpi)
